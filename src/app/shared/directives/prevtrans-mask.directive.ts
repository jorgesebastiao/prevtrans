...
@Directive({
  selector: '[prevtransMask]',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: PrevtransMaskDirective,
    multi: true
  }]
})
export class PrevtransMaskDirective implements ControlValueAccessor {

  onTouched: any;
  onChange: any;

  @Input('prevtransMask') prevtransMask: string;
  ...
  onKeyup($event: any) {
    ...
    let pad = this.prevtransMask.replace(/\D/g, '').replace(/9/g, '_');
  ...
  onBlur($event: any) {
    if ($event.target.value.length === this.prevtransMask.length) {
      return;
    }
    this.onChange('');
    $event.target.value = '';
